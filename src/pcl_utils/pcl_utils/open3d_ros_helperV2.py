...
#modified -> used the read_points() from point_cloud2
def rospc_to_o3dpc(rospc, remove_nans=False):
    ...
    points = list(pc2.read_points(rospc, field_names=field_names, skip_nans=remove_nans))
...
def o3dpc_to_rospc(o3dpc, frame_id=None, stamp=None):
    ...
    for field in data.dtype.names:
        ...
        count = 1
        rospc.fields.append(PointField(name=field, offset=offset, datatype=datatype, count=count))
...
def ppf_icp_registration(source_cloud, target_cloud, n_points=3000, n_iter=100, tolerance=0.001, num_levels=5, scale=0.001):
    ...
    icp_fnc = cv2.ppf_match_3d_ICP(n_iter, tolerance=tolerance, rejectionScale=2.5, numLevels=num_levels) 
    try:
