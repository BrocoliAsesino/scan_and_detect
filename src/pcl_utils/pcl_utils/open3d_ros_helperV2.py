...
def se3_to_transform_stamped(transform_nparray, source_frame, target_frame, stamp=None):
    ...
    if stamp is None:
        stamp = Clock().now().to_msg()
    transform_stamped = pq_to_transform_stamped(
        pos, quat, source_frame, target_frame, stamp
    )
    return transform_stamped
...
def convert_rgbUint32_to_tuple(rgb_uint32):
    return (
        (int(rgb_uint32) & 0x00FF0000) >> 16,
        (int(rgb_uint32) & 0x0000FF00) >> 8,
        (int(rgb_uint32) & 0x000000FF),
    )


def pointcloud2_to_o3d(rospc):
    ...
    points = list(pc2.read_points(rospc, field_names=field_names, skip_nans=True))

    o3dpc = open3d.geometry.PointCloud()
    xyz = np.array([(point[0], point[1], point[2]) for point in points])

    if len(xyz) > 0:
        o3dpc.points = open3d.utility.Vector3dVector(xyz)

        if is_rgb:
            rgb = np.array([convert_rgbUint32_to_tuple(point[3]) for point in points])
            o3dpc.colors = open3d.utility.Vector3dVector(rgb / 255.0)
    return o3dpc
...
def load_stl_as_point_cloud(file_path, scale_factor=0.001):
    if not os.path.isfile(file_path):
        print(f"File not found: {file_path}", flush=True)
        return None

    mesh = open3d.io.read_triangle_mesh(file_path)
    mesh.scale(scale_factor, np.array([0, 0, 0]))
...
def o3dpc_to_rospc(o3dpc, frame_id=None, stamp=None):
    ...
    if is_color:
        ...
        data = np.zeros(
            n_points, dtype=[("x", np.float32), ("y", np.float32), ("z", np.float32)]
        )
    data["x"] = cloud_npy[:, 0]
    data["y"] = cloud_npy[:, 1]
    data["z"] = cloud_npy[:, 2]
    ...
    if is_color:
        ...
        rgb_npy = (
            rgb_npy[:, 0] * BIT_MOVE_16 + rgb_npy[:, 1] * BIT_MOVE_8 + rgb_npy[:, 2]
        )
    ...
    if stamp is None:
        ...
        header.stamp = stamp
    rospc.header = header
    rospc.header.stamp = time_stamp.to_msg()
    rospc.height = 1
    rospc.width = n_points
    rospc.fields = []
    for field in data.dtype.names:
        offset = data.dtype.fields[field][1]
        datatype = PointField.FLOAT32
        if field == "rgb":
            datatype = PointField.UINT32
        count = 1
        rospc.fields.append(
            PointField(name=field, offset=offset, datatype=datatype, count=count)
        )
...
def apply_pass_through_filter(o3dpc, x_range, y_range, z_range):
    ...
    x_range = np.logical_and(
        cloud_npy[:, 0] >= x_range[0], cloud_npy[:, 0] <= x_range[1]
    )
    y_range = np.logical_and(
        cloud_npy[:, 1] >= y_range[0], cloud_npy[:, 1] <= y_range[1]
    )
    z_range = np.logical_and(
        cloud_npy[:, 2] >= z_range[0], cloud_npy[:, 2] <= z_range[1]
    )
...
def p2p_icp_registration(
    ...
    icp_result = open3d.registration.registration_icp(
        source=source_cloud,
        target=target_cloud,
        max_correspondence_distance=max_correspondence_distance,  # unit in millimeter
        init=np.eye(4),
...
def ppf_icp_registration(
    ...
    source_np_cloud = np.concatenate(
        [np.asarray(source_cloud.points), np.asarray(source_cloud.normals)], axis=1
    ).astype(np.float32)
    target_np_cloud = np.concatenate(
        [np.asarray(target_cloud.points), np.asarray(target_cloud.normals)], axis=1
    ).astype(np.float32)
    icp_fnc = cv2.ppf_match_3d_ICP(
        n_iter, tolerance=tolerance, rejectionScale=2.5, numLevels=num_levels
    )
    try:
        retval, residual, pose = icp_fnc.registerModelToScene(
            source_np_cloud, target_np_cloud
        )
    except Exception:
        return None, 10000
    else:
