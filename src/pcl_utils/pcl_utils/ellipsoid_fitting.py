...
def project_point_cloud_to_plane(
    pcd: o3d.geometry.PointCloud, plane_model: np.ndarray
) -> np.ndarray:
    """
...
def extract_2d_convex_hull(
    ...
    if len(points_2d) < 5:
        raise ValueError(
            f"Need at least 5 points for ellipse fitting, got {len(points_2d)}"
        )
...
def fit_ellipse_to_2d_points(
    perimeter_points_2d: np.ndarray,
    ...
    if len(perimeter_points_2d) < 5:
        raise ValueError(
            f"cv2.fitEllipse requires at least 5 points, got {len(perimeter_points_2d)}"
        )
...
def create_upper_ellipsoid_mesh(
    ...
    sphere.triangles = o3d.utility.Vector3iVector(
        np.array(valid_triangles, dtype=np.int32)
    )
...
def generate_camera_viewpoints_around_ellipsoid(
    center_3d: np.ndarray,
    axes_3d: tuple[float, float, float],
    rotation_matrix: np.ndarray,
...
def generate_camera_viewpoints_along_principal_axis(
    center_3d: np.ndarray,
    axes_3d: tuple[float, float, float],
    rotation_matrix: np.ndarray,
    plane_model: np.ndarray,
    ...
    elevation_angles = np.linspace(
        0.33 * np.pi, -0.33 * np.pi, num_viewpoints, endpoint=False
    )
...
def plot_ellipse_fit(
    ...
    plt.scatter(
        perimeter_points[:, 0], perimeter_points[:, 1], c="blue", s=50, zorder=5
    )
...
def display_object_with_ellipsoid(
    pcd: o3d.geometry.PointCloud,
...
def visualize_viewpoints_around_object(
    ...
    print(
        f"Object center: ({center_3d[0]:.3f}, {center_3d[1]:.3f}, {center_3d[2]:.3f})"
    )
...
def ellipsoid_fitting_pipeline(
    pcd: o3d.geometry.PointCloud, plane_model: np.ndarray, debug: bool = False
) -> Tuple[
    np.ndarray, Tuple[float, float, float], np.ndarray, o3d.geometry.TriangleMesh
]:
    """
    ...
    if debug:
        visualize_viewpoints_around_object(
            pcd=pcd,
